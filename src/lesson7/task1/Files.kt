    ...
    for (line in File(inputName).readLines()) {
        for (findWord in substrings) {
            ...
            while (subIndex != -1) {
                ...
                subIndex = line.indexOf(findWord, subIndex + 1, true)
            }
        }
    }
    return res
...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    if (maxLong != 0) {
        for (line in subTextFile) {
            if (line.isNotEmpty()) {
                val subLine = line.split(' ')
                if (subLine.size < 2) {
                    writer.write(subLine[0])
                    writer.newLine()
                    continue
                }
                val countProb = maxLong - line.length
                val countRepeat = countProb / (subLine.size - 1)
                var countSubProb = countProb % (subLine.size - 1)
                for (wordIndex in 0..subLine.size - 2) {
                    writer.write(subLine[wordIndex] + " ")
                    writer.write(" ".repeat(countRepeat))
                    if (countSubProb != 0) {
                        writer.write(" ")
                        countSubProb--
                    }
                }
                writer.write(subLine[subLine.size - 1])
                writer.newLine()
            } else writer.newLine()
        }
    }
