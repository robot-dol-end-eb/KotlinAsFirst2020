...
fun sin(x: Double, eps: Double): Double {
    ...
    while (eps < abs(xn)) {
        result += xn
        xn = if (sign)
            subX.pow(coef) / factorial(coef)
        else
            subX.pow(coef) / factorial(coef) * (-1)
        coef += 2
        sign = !sign
    }
    return result
...
fun squareSequenceDigit(n: Int): Int {
    ...
    var rezult = 0
    while (subLength < n) {
        subNumb = number * number
        number++
        subLength += digitNumber(subNumb)
    }
    for (i in n..subLength) {
        rezult = subNumb % 10
        subNumb /= 10
    }
    return rezult
