...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        res[grade] = (res[grade] ?: emptyList()) + name
    }
    return res
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((stock, cost) in stockPrices) res[stock] = (res[stock] ?: 0.0) + cost
    for ((stock, cost) in res) res[stock] = res[stock]!! / stockPrices.count { it.first == stock }
    return res;
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (item in list) res[item] = (res[item] ?: 0) + 1
...
fun hasAnagrams(words: List<String>): Boolean {
    val sublist = mutableSetOf<Set<Char>>()
    for (name in words)
        sublist.add(name.toSet())
    return words.size != sublist.size
}
